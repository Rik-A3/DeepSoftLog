...
def generate_prolog_files():
    ...
    for held_out in range(100,1000,100):
        for problem in (f'S{i}' for i in range(1,4)):
            data = load_tsv_file(base_path / f"raw/countries_{held_out}/countries_{problem}_merged.tsv")
            data = data_to_prolog(data, name="countries")
            file_str = [f"{query.query}." for query in data]
            with open(base_path / f"tmp/countries_{held_out}_{problem}.pl", "w+") as f:
                f.write("\n".join(file_str))
